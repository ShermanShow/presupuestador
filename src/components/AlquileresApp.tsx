...
export default function AlquileresApp() {
  ...
  // Edición de un presupuesto guardado (traído desde el historial)
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editingNumero, setEditingNumero] = useState<string | null>(null);

  function aplicarPresupuestoGuardado(p: any, list: Equipo[]) {
    let eq: Equipo | null = null;
    if (p.equipo_id) eq = list.find((e) => e.id === p.equipo_id) || null;
    if (!eq && p.equipo_snapshot) {
      eq = { ...p.equipo_snapshot, id: p.equipo_snapshot.id || `guardado-${p.id}`, activo: true } as Equipo;
    }
    if (eq) {
      setEquipos((prev) => (prev.some((e) => e.id === eq!.id) ? prev : [eq!, ...prev]));
      setSelectedId(eq.id);
      setColorCpc(eq.cpcColorUsd || 0);
    }
    if (p.cliente) setClient(p.cliente);
    if (p.vendedor) setSeller(p.vendedor);
    if (typeof p.renta_final_ars === "number") setFinalRent(p.renta_final_ars);
    if (typeof p.validez_dias === "number") setValidity(p.validez_dias);
    if (p.observaciones) setNotes(p.observaciones);
    if (typeof p.amortizacion_meses === "number") setAmort(p.amortizacion_meses);
    if (typeof p.dolar === "number") setDolar(p.dolar);
    if (typeof p.copias_bn_incluidas === "number") setBnCopies(p.copias_bn_incluidas);
    if (typeof p.precio_bn_usd === "number") setBnUsd(p.precio_bn_usd);
    if (typeof p.copias_color_incluidas === "number") setColorCopies(p.copias_color_incluidas);
    if (typeof p.cpc_color_usd === "number") setColorCpc(p.cpc_color_usd);
    if (typeof p.multiplicador_color === "number") setColorMult(p.multiplicador_color);
    setEditingId(p.id || null);
    setEditingNumero(p.numero || null);
  }

  useEffect(() => { Promise.all([fetch("/api/alquileres/equipos").then(r => r.json()), fetch("/api/cotizacion-bna").then(r => r.ok ? r.json() : null)]).then(([data, quote]) => { const list = Array.isArray(data.equipos) ? data.equipos : []; setEquipos(list); setSource(data.source || "demo"); if (data.config) { setConfig(data.config); setAmort(data.config.amortizacionMeses); setDolar(data.config.dolar); setBnUsd(data.config.precioBnUsd); setColorMult(data.config.multiplicadorColor); } if (quote?.venta) { setBna(quote.venta); setDolar(quote.venta); } if (list[0]) { setSelectedId(list[0].id); setColorCpc(list[0].cpcColorUsd || 0); } try { const editarRaw = sessionStorage.getItem("presupuesto_editar"); if (editarRaw) { sessionStorage.removeItem("presupuesto_editar"); const p = JSON.parse(editarRaw); if (p?.equipo_snapshot || p?.cliente) aplicarPresupuestoGuardado(p, list); } } catch (err) { console.error("Error pre-cargando presupuesto:", err); } }).catch(() => setSaved("No se pudo cargar el catálogo.")).finally(() => setLoading(false)); }, []);
